...
def _normalize(raw: dict) -> dict:
    ...
    cfg["manual_password"] = str(cfg.get("manual_password") or "").strip()
...
class RegisterService:
    ...
    def update(self, updates: dict) -> dict:
        with self._lock:
            ...
            openai_register.config.update({k: self._config[k] for k in ("mail", "proxy", "total", "threads", "manual_password")})
            self._save()
            return self.get()
